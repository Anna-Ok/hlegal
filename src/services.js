...
window.addEventListener('load', () => {
  setTimeout(() => {
    ...
    // Start scroll reveal after preloader fades so above-fold animations are visible
    startReveal();
  }, 1400);
});
...
function startReveal() {
  ...
  requestAnimationFrame(() => {
    document.querySelectorAll('.services__item').forEach((el, i) => {
      el.setAttribute('data-reveal', '');
      el.dataset.delay = 200 + i * 80;
      revealObserver.observe(el);
    });
    document.querySelectorAll('.clients__item').forEach((el, i) => {
